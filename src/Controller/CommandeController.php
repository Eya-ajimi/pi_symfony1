...
final class CommandeController extends AbstractController
{
    ...
    public function showCommandePayee(CommandeRepository $commandeRepository): Response
    {
        ...
        $utilisateur->setId($this->currentId);
    ...
    public function commandeDetails(int $commandeId, int $shopId, CommandeRepository $commandeRepository): Response
    {
        ...
        if ($paniersFiltered[0] == null) {
            $paniersFiltered[2] = $paniersFiltered[1];
        } else {
            $paniersFiltered[2] = $paniersFiltered[0];
        }

        return $this->render('maria_templates/CommandeWithDetails.html.twig', [
            'commande' => [
                'ticketNumber' => $paniersFiltered[2]->getNumeroTicket(),
                'id' => $commande->getId(),
                'client' => [
